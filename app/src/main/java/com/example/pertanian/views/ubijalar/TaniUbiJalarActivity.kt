...
import com.example.pertanian.viewmodels.ubijalar.TaniUbiJalarViewModel
import kotlinx.android.synthetic.main.activity_tani_jagung.*

class TaniUbiJalarActivity : AppCompatActivity(), TaniRcyAdapter.OnItemClickListener {

    lateinit var taniRcyAdapter: TaniRcyAdapter
    lateinit var taniUbiJalarViewModel: TaniUbiJalarViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar!!.title = "Produktivitas UbiJalar"
        ...
        btnAddPerikanan.setOnClickListener {
            val intent = Intent(this, TambahEditUbiJalarActivity::class.java)
    ...
    private fun initViewModel() {
        taniUbiJalarViewModel = ViewModelProvider(this).get(TaniUbiJalarViewModel::class.java)
        taniUbiJalarViewModel.getUbiJalarLiveObserveable().observe(this, Observer<PertanianList>{
            if(it == null){
                Toast.makeText(this@TaniUbiJalarActivity, "No result found", Toast.LENGTH_LONG).show()
            }else{
        ...
        editTextPerikananSearch.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
               if(s!!.length > 0){
                   ...
                   taniRcyAdapter.notifyDataSetChanged()
                   taniUbiJalarViewModel.getUbiJalarList(s.toString())
               }else{
                   taniRcyAdapter.taniList.clear()
                   taniRcyAdapter.notifyDataSetChanged()
                   taniUbiJalarViewModel.getUbiJalarList("")
               }
            }
    ...
    private fun initRecyclerView() {
        rcyPerikanan.apply {
            layoutManager = LinearLayoutManager(this@TaniUbiJalarActivity)
            taniRcyAdapter = TaniRcyAdapter(this@TaniUbiJalarActivity)
